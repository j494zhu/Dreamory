...
from app import clock             # 注入时钟模块

from .state import (
    AFFECTION_MAX,  # 好感度上限
    AFFECTION_MIN,  # 好感度下限
    AffectState,    # 情绪状态对象, [运行时状态, 可变], 也是dynamics的加工对象. 包含mode(6态), 标量(arousal/security/patience/affection), 多轴, 回路旧账自我归因等等.
    Grievance,      # 旧账数据类, 有三个字段: content, weight, resolved
    OpenLoop,       # 挂起回路数据类, 包含被忽略的投标, 未兑现的承诺. 包含type, content, weight, sessions_old, due_ms字段
    affection_tier, # 将好感度对应到文字层级的函数. 输入为0-200的数值, 返回("lover", "恋人")这样的元组
)
from .persona import Persona      # 人格超参数, [编译期常量, 只读]. (anxieity, avoidance, withdrawn, expressiveness, insight, confabulation 等.)

# ── 可调常数(集中放,方便做参数扫描)──────────────────────────
AROUSAL_HALFLIFE_S = 45 * 60      # 情绪激活半衰期 45 分钟 - 情绪激活度半衰期(单位: 秒)
SESSION_GAP_S = 6 * 3600          # 超过 6 小时, apply_time 就判定新会话 - 重置patience, 年老化回路计数, 可能触发旧账沉淀, 是唯一的session更新规则(单位: 秒)
SEC_HIT_TURN_AWAY = 0.05          # turn_away 砸在 bid 上的安全感损失(需要 x anxiety) - 投标被忽略以后安全感扣多少的常量参数
SEC_HIT_TURN_AGAINST = 0.10       # 被攻击以后安全感下降的参数常量
SEC_GAIN_TURN_TOWARD = 0.02       # 非对称:正面增益约为负面冲击的一半以下 - 投标被借助以后安全感增加参数常量
SEC_GAIN_REPAIR = 0.05            # 修复被接受以后安全感上升的参数常量
SEC_GAIN_LOOP_CLOSED = 0.04       # 挂起回路被解决以后, 安全感提升0.04
REPAIR_ACCEPT_BASE = 0.35         # security 高于此值才可能接受修复(×avoidance 调高)
REPAIR_PITY_ATTEMPTS = 3          # 连续哄到第几次,即使 security 不够也保底松动(避免"哄不动"死锁) - 留给各位直男的后门.
WITHDRAW_PRESSURE = 5             # loop 总压力超过此值 → withdrawn(÷avoidance 调低) - 挂起回路总权重超过这个值以后, 态度变成widthdrawn(不想说话了)
PROBING_SECURITY = 0.40           # security 低于此值且出现模糊信号 → probing - security过低, 讲话又不清楚就会让llm变得更加试探
LOOP_ESCALATE_SESSIONS = 1        # 挂起回路熬过几个会话后沉淀为旧账
MIN_TURNS_IN_NEG_MODE = 2         # 滞回:负面模式最短停留轮数, 防止在临界值周围震荡. 

# ── 好感度(affection,0~200)常数 ────────────────────────────
# 定位:security 是"此刻安不安全",affection 是"我们走到哪一步了"。
# 涨得慢(每轮零点几到几点),跌得比涨快(×anxiety),高段位增益递减(越熟越难再刷)。
AFF_GAIN_TURN_TOWARD = 0.8        # 投标被接住, -------------------------------------------加分--
...
AFF_GAIN_LOOP_CLOSED = 1.5        # 他记得并回应了她挂着的事 -------------------------------加分--
AFF_HIT_TURN_AWAY = 1.2           # 投标被忽略(×anxiety)-----------------------------------减分--
AFF_HIT_TURN_AGAINST = 3.0        # 攻击性回应(×anxiety)
AFF_HIT_GRIEVANCE = 2.0           # 挂起回路沉淀成旧账时,每条额外掉的好感--------------------减分--
AFF_HIGH_GAIN_KNEE = 100.0        # 好感度高于此值(恋人档)后,正向增益开始递减
AFF_HIGH_GAIN_FLOOR = 0.3         # 好感度递减的保底(200 时仍保留 30% 增益)
AFF_DECAY_GRACE_DAYS = 3.0        # 离线超过此天数,好感开始缓慢回落(冷落是有代价的)---------除你旮旯给木账号!
AFF_DECAY_PER_DAY = 1.0           # 超出宽限期后每天回落多少
AFF_DECAY_FLOOR = 60.0            # 回落下限:感情会淡,但共同经历不会清零(只降到"友好"档)----别真成普瑞塞斯了.  
...
ADRENALINE_HALFLIFE_S = 20 * 60   # 肾上腺素 (3 * 60)   , fight or flight
OXYTOCIN_HALFLIFE_S = 3 * 3600    # 催产素   (10 * 60)  , 拥抱激素
CORTISOL_HALFLIFE_S = 20 * 3600   # 皮质醇   (2 * 3600) , 压力激素

ADR_TURN_AGAINST = 0.40           # 被攻击:肾上腺素飙--------------------------------------肾上腺素--
ADR_EXCITED = 0.20                # 狂喜/兴奋的好事也会心跳加速
ADR_MILESTONE = 0.30              # 跨过关系里程碑那一刻,心怦怦跳---------------------------肾上腺素--
...
COR_BROKEN_PROMISE = 0.15         # 爽约的压力残留(皮质醇)

# ── 旧账和解 ──────────────────────────────────────────────────
# 旧账不随时间消失,但也不该永远无法翻篇:他带着歉意/补偿直面那件事
# (extractor: topic_relates_to_grievance_id + is_repair_attempt),过了修复
# 门槛才算和解;心结太深哄不动时,示好到第 GRIEVANCE_PITY_TOUCHES 次保底翻篇
# (与修复的"直男后门"同构,防死锁)。
GRIEVANCE_PITY_TOUCHES = 3        # 对同一本旧账真心示好这么多次,保底翻篇
AFF_GAIN_GRIEV_RESOLVED = 2.0     # 和解一本旧账,与真心和好同级-------------------------加分--
AFF_GAIN_GRIEV_PITY = 0.8         # 保底和解:这一页翻了,但心里没真的暖回来
SEC_GAIN_GRIEV_RESOLVED = 0.05    # "他记得,他在乎"的安全感
OXY_GRIEV_RESOLVED = 0.20         # 心结解开后的柔软余韵
COR_RELIEF_GRIEV_RESOLVED = 0.10  # 压着的石头落地(对称:沉淀一条旧账时 cortisol +0.10)

# ── 轻回路自然遗忘 ────────────────────────────────────────────
# weight<3 的回路沉不成旧账(见 _escalate_old_loops),但此前会永远挂着——
# 鸡毛蒜皮不进旧账,却以【挂起回路】的形式无限碎碎念。熬过几个会话就该淡忘。
LOOP_FORGET_SESSIONS = 2          # 轻回路(weight<3)熬过几个会话后不再挂心


# 高好感度收益递减函数, 100(恋人)以下不折损, 100-200(恋人以上)线性从100%递减到30%
...
# 实际好感度收益 = amount * (1 + 25% * 催产素) * _gain_scale()
...
# apply_time() 把状态从上次记录的时间点, 快进到现在. 每次 pipeline 处理消息之前先调用 apply_time()
def apply_time(state: AffectState, persona: Persona, now: float | None = None) -> None:
    ...
    # 好感度的离线回落:被晾了太多天,感情会淡一点,但不会跌穿"友好"档
    # (共同的经历还在,只是热度凉了)。
    ...
    if gap > SESSION_GAP_S: # 超过一定时间, 跨会话了
        # 新会话:耐心重置(好感度折算加成;隔夜没散的压力磨掉一截),
        # warm_streak 落回好感度决定的底座
        cortisol_drag = int(round(COR_PATIENCE_DRAG * state.cortisol)) # 高皮质醇会掉耐心
...
def _escalate_old_loops(state: AffectState, now_s: float) -> None:
    ...
    remain = []
    for loop in state.open_loops:
        if loop.type == "commitment": # "commitment" 或者 "unanswered_bid"
            overdue = (loop.due_ms is not None
                       and now_ms_v > loop.due_ms + COMMITMENT_GRACE_H * 3600_000) # 1hr = 3600000 ms
        ...
        elif loop.weight < 3 and loop.sessions_old >= LOOP_FORGET_SESSIONS:
            pass   # 轻回路自然淡忘:鸡毛蒜皮的事,过两晚就不挂心了(不沉旧账,不惩罚)
        else:
            remain.append(loop)
...
def apply_events(state: AffectState, ev: dict, persona: Persona,
    ...
    # ── 6. 旧账被话题触碰:带着歉意直面 → 有机会和解;只是提起 → arousal 上浮 ──
    # 和解门槛复用 repair_threshold(回避型更难翻篇、感情深容易心软、激素调制),
    # 哄到第 GRIEVANCE_PITY_TOUCHES 次保底翻篇 —— 与规则 4 的保底松动同构。
    if ev["topic_relates_to_grievance_id"]:
        g = state.find_grievance(ev["topic_relates_to_grievance_id"])
        if g and ev["is_repair_attempt"]:
            g.touches += 1
            genuine = state.security > repair_threshold(state, persona)
            if genuine or g.touches >= GRIEVANCE_PITY_TOUCHES:
                g.resolved = True
                state.security = min(1.0, state.security + SEC_GAIN_GRIEV_RESOLVED)
                state.cortisol = max(0.0, state.cortisol - COR_RELIEF_GRIEV_RESOLVED)
                if genuine:
                    _aff_gain(state, AFF_GAIN_GRIEV_RESOLVED)
                    _hormone(state, "oxytocin", OXY_GRIEV_RESOLVED)
                    trace.append(f"旧账和解,这一页翻篇了: {g.content}")
                else:
                    _aff_gain(state, AFF_GAIN_GRIEV_PITY)
                    trace.append(f"旧账勉强翻篇(说到第{g.touches}次,算了): {g.content}")
            else:
                state.arousal = min(1.0, state.arousal + 0.1)
                trace.append(f"他想把这本旧账说开(第{g.touches}次),她心里还没过去")
        else:
